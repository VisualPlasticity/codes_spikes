...
% much easier!
%
function [dataArray,samplrate,starttime,endtime] = GLXDataMatch(binName,path,samplingrate,template,channels2take,dataType)
% dataType =  A or D (char)
if nargin<1
% Ask user for binary file
[binName,path] = uigetfile('*.bin', 'Select Binary File');
end

% Length of template in time (seconds)
duration = length(template)./samplingrate;

% Parse the corresponding metafile
meta = ReadMeta(binName, path);
% Get first one second of data
samplrate = SampRate(meta);
DownsRate = samplrate./samplingrate;
nSamp = (duration *1.1 * SampRate(meta)); %Take a little bit larger timewindow
totalsamps = str2num(meta.fileSizeBytes);

dw = 1; % For a digital channel: read this digital word dw in the saved file
% (1-based). For imec data there is never more than one saved digital word.
startsample = 1;
while startsample<totalsamps
    dataArray = ReadBin(startsample, startsample+nSamp-1, meta, binName, path);    
    if dataType == 'A'
        if strcmp(meta.typeThis, 'imec')
            dataArray = GainCorrectIM(dataArray, [channels2take], meta);
        else
            dataArray = GainCorrectNI(dataArray, [channels2take], meta);
        end
    else
        dataArray = ExtractDigital(dataArray, meta, dw, channels2take);
    end
    dataArray = downsample(dataArray,uint16(DownsRate));
    syncDatImec(:,idx:idx+length(dataArray)-1)=dataArray;
    startsample = startsample+nSamp;
    idx = 1+length(dataArray);
    if startsample>totalsamps
        break
    end
end
...
dataArray = dataArray(:,startpoints(SessionIndex):endpoints(SessionIndex));
starttime = startpoints(SessionIndex)./ SampRate(meta);
endtime = endpoints(SessionIndex)./ SampRate(meta);
...
function dataArray = GainCorrectNI(dataArray, chanList, meta)
    ...
    for i = 1:length(chanList)
        conv = fI2V / ChanGainNI(chanList(i), MN, MA, meta);
