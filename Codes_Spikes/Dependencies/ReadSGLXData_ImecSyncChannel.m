...
% much easier!
%
function [dataArray,samplrate]= ReadSGLXData_ImecSyncChannel(binName,path,ch)
if nargin<1
...
nSamp = str2num(meta.fileSizeBytes);
% Get first x seconds of data (at a time)
startsample = 0;
...
dataArray = ReadBin(startsample, nSamp, meta, binName, path);
if dataType == 'A'
...
sizeA = [length(synchan), nSamp];

fid = fopen(fullfile(path, binName), 'rb');
fseek(fid, samp0 * 2 * synchan-1, 'bof');
...
for i = 1:numel(dLineList)
    digArray(i,:) = bitget(dataArray(1,:), dLineList(i)+1, 'int16');
end
end % ExtractDigital
