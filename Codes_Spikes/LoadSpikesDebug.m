...
myKsDir = 'C:\Users\EnnyB\Documents\Data\KiloSortOutput\EB001\2021-02-25'
myLFDir = '\\znas\Subjects\EB001\2021-02-25\ephys\'
...
dataArray = ReadSGLXData(mySyncFile(1).name,mySyncFile(1).folder);
%             syncDat = extractSyncChannel(mySyncFile(1).folder, mySyncFile,nChansInFile, syncChanIndex);

eventTimes = spikeGLXdigitalParse(syncDat, lfpFs);

%Remove empty times
eventTimes(cell2mat(cellfun(@(X) isempty(X{1}),eventTimes,'UniformOutput',0)))=[];
