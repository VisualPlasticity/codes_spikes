function SS = stimSparseNoiseAsync_Luminance(myScreenInfo,Pars)
...
% 2021-05 EB - add option to make it less bright (luminance) as mice don't
% like high luminance
%% Basics
...
pp{12} =  {'l',	     'Mean luminance (%)',                     40,0,100};


x       = XFile('stimSparseNoiseAsync_Luminance', pp);
...
noise = noise+0.5; %between 0 and 1
noise = noise.*mm; % apply luminance
noise = noise-0.5; % back to between -0.5 and 0.5
images = noise;
...
SS = stimSparseNoiseAsync_Luminance(myScreenInfo); 
